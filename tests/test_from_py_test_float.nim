...
import float_utils
import float_utils/parsefloat

import pkg/pyunittest
when defined(js):
  import pkg/nimpatch/jsBiggestIntTruediv

from pkg/pymath import ldexp

func float(a: string): BiggestFloat =
  ## `PyFloat_FromString`.
  ## the same as `float(str|bytes)`
  # used by builtins.complex
  let ni = parsePyFloat(a, result)
  if ni != a.len:
    raise newException(ValueError,
      "could not convert string to float: " & repr(a))

template range(stop: int): untyped = 0..<stop
...
suite "float":
    ...
    test "as_integer_ratio":
        ...
        check (0, 1) == (0.0).as_integer_ratio()
        check (5, 2) == (2.5).as_integer_ratio()
        check (1, 2) == (0.5).as_integer_ratio()
        when false and sizeof(system.int) > 4: # cannot pass
            check ((system.int 4728779608739021, system.int 2251799813685248) ==
                            ((2.1).as_integer_ratio()))
            check ((system.int -4728779608739021, system.int 2251799813685248) ==
                            ((-2.1).as_integer_ratio()))
        check (-2100, 1) == (-2100.0).as_integer_ratio()
